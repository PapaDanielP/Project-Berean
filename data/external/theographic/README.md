...
This directory is a metadata-only declaration for the external upstream repository `robertrouse/theographic-bible-metadata`, described upstream as a knowledge graph of biblical people, places, periods, and passages.

The upstream repository is not vendored here. No upstream CSV, JSON, Neo4j, API, geography, documentation, or other data files are copied into Berean by this declaration.

Berean pins this external source to commit `cfb1c485d4da6fb63a69cb3b7f5b0752792f46bc` on the upstream `master` branch. The pin date is August 10, 2026, which records Berean's inspection/pin date and is distinct from the upstream commit timestamp.
