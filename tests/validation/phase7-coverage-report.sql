...
\echo 'Phase 7 Genesis 1-11 chapter coverage'
WITH chapters AS (
...
BEGIN
    IF (
        SELECT count(*)
        FROM source_record
        WHERE source_record_key IN ('MT_GEN_1_6', 'MT_GEN_1_7', 'MT_GEN_1_8', 'MT_GEN_1_9')
    ) <> 4 THEN
        RAISE EXCEPTION 'phase7 coverage: Genesis 1:6-9 batch is absent';
    END IF;
END $$;
