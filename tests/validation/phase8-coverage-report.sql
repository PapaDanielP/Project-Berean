...
BEGIN
    ...
    -- Genesis 1:14-31 must remain explicitly deferred; chapter 1 must not be reported complete.
    IF EXISTS (
        SELECT 1
        FROM source_record
        WHERE source_record_key LIKE 'MT_GEN_1\_%' ESCAPE '\'
          AND substring(source_location FROM ':([0-9]+)$')::int >= 14
    ) THEN
        RAISE EXCEPTION 'phase8 coverage: Genesis 1:14-31 must remain deferred for this batch';
    END IF;
END $$;
